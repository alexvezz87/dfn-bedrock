...
function cv_aggiungi_bottone_pagamento_email($order, $sent_to_admin, $plain_text, $email)
{
    ...
    if ($email->id === 'customer_invoice') {
        ...
        echo '<div style="text-align: center; margin: 40px 0; padding: 25px; border: 2px solid #d63638; border-radius: 8px; background-color: #fffafb;"><h3 style="color: #d63638; margin-top: 0; font-size: 22px;">⚠️ I tuoi posti sono riservati temporaneamente</h3><p style="font-size: 16px; color: #333; margin-bottom: 25px;">Abbiamo riservato i tuoi posti, ma questa prenotazione scadrà automaticamente tra <strong>24 ore</strong>. Se il contributo non verrà versato entro questo termine, la prenotazione verrà annullata in automatico e i posti torneranno liberi.</p><a href="' . esc_url($tracked_url) . '" style="background-color: #7016d9; color: #ffffff; padding: 18px 36px; font-size: 20px; font-weight: bold; text-decoration: none; border-radius: 8px; display: inline-block; box-shadow: 0 4px 10px rgba(0,0,0,0.15);">💳 VERSA IL CONTRIBUTO E CONFERMA</a></div>';
    }
}
