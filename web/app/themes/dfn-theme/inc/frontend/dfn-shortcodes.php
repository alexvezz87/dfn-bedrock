...
function dfn_render_evento_shortcode($atts): string
{
    ...
    $is_past = ($event->event_date_start < $today) || ($event->event_date_start === $today && ! empty($ev_time) && $ev_time < $now);
    ...
    <?php if ($effective_layout === 'layout2') : ?>
        ...
        <div class="dfn-layout2-wrapper">
            ...
            <div class="dfn-layout2-content">
                ...
                <?php elseif ($is_past) : ?>
                    <div class="dfn-sold-out-card" style="background:#f8fafc; border:1px solid #cbd5e1;">
                        <div class="dfn-sold-out-icon">🏁</div>
                        <h3 class="dfn-sold-out-title" style="color:#1e293b;"><?php esc_html_e('Evento Concluso', 'dfn-theme'); ?></h3>
                        <p class="dfn-sold-out-text" style="color:#64748b;"><?php printf(esc_html__('Questo evento si è svolto il %s. Le prenotazioni online sono terminate.', 'dfn-theme'), esc_html(date_i18n('d F Y', strtotime($event->event_date_start)))); ?></p>
                    </div>
