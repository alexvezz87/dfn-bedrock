...
function cv_render_pagina_liste_attesa()
{
    ...
    if (isset($_POST['cv_add_waitlist_nonce']) && wp_verify_nonce($_POST['cv_add_waitlist_nonce'], 'cv_add_waitlist')) {
        ...
        if (is_email($email) && $selected_event > 0 && $qty > 0) {
            ...
            ob_start();
            wc_get_template('emails/email-header.php', [ 'email_heading' => 'Sei in Lista d\'Attesa' ]);
            ...
            WC()->mailer()->send($email, 'Conferma inserimento in Lista d\'Attesa - ' . $nome_evento, $message, [ 'Content-Type: text/html' ]);
