...
{
    if ($hook !== 'toplevel_page_cv-scanner-live') {
        return;
    }

    wp_enqueue_style('cv-scanner-css', get_stylesheet_directory_uri() . '/assets/css/cv-scanner.css', [], '1.0');

    // Libreria esterna QR Code
    wp_enqueue_script('html5-qrcode', 'https://unpkg.com/html5-qrcode@2.3.8/html5-qrcode.min.js', [], null, false);
    wp_enqueue_script('cv-scanner-js', get_stylesheet_directory_uri() . '/assets/js/cv-scanner.js', ['html5-qrcode', 'jquery'], '1.0', true);

    wp_localize_script('cv-scanner-js', 'cvScannerVars', [
