...
function cv_enqueue_botteghino_assets($hook)
{
    ...
    wp_enqueue_style('cv-botteghino-css', get_stylesheet_directory_uri() . '/assets/css/cv-botteghino.css', [], '1.0');
    wp_enqueue_script('cv-botteghino-js', get_stylesheet_directory_uri() . '/assets/js/cv-botteghino.js', ['jquery', 'selectWoo'], '1.0', true);
...
function cv_render_generatore_fai()
{
    ...
    if (isset($_POST['cv_genera_fai_nonce']) && wp_verify_nonce($_POST['cv_genera_fai_nonce'], 'cv_genera_fai')) {
        ...
        if ($prodotto_id === CV_EVENT_ID_STANDARD) {
            $tessere = 0;
        }
