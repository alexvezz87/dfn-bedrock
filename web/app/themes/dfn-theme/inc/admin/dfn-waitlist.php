...
function dfn_render_waitlist_page(): void {
    ...
    if ( isset( $_POST['dfn_waitlist_action_nonce'] ) && wp_verify_nonce( $_POST['dfn_waitlist_action_nonce'], 'dfn_waitlist_action' ) ) {
        ...
        if ( $entry ) {
            if ( $action === 'delete' ) {
                $wpdb->delete( $table_waitlist, array( 'id' => $entry_id ), array( '%d' ) );
                ...
                if ( $product ) {
                    ...
                    try {
                        ...
                        $wpdb->insert(
                            ...
                            array( '%d', '%d', '%s', '%s', '%s', '%d', '%d', '%d', '%s', '%s', '%s', '%f', '%f' )
                        );
                        ...
                        if ( $entry->slot_id ) {
                            ...
                            if ( $slot ) {
                                $wpdb->update(
                                    ...
                                    array( 'booked_count' => intval( $slot->booked_count ) + intval( $entry->persons ) ),
                                    array( 'id' => $entry->slot_id ),
                                    array( '%d' ),
                                    array( '%d' )
                                );
                        ...
                        $wpdb->update(
                            $table_waitlist,
                            array( 'status' => 'promoted', 'promoted_order_id' => $order->get_id() ),
                            array( 'id' => $entry_id ),
                            array( '%s', '%d' ),
                            array( '%d' )
                        );
