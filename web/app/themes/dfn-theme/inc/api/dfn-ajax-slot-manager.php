...
function dfn_ajax_admin_verify_access(): void {
    if ( ! current_user_can( 'manage_options' ) && ! current_user_can( 'edit_pages' ) ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Permessi insufficienti.', 'dfn-theme' ) ) );
    }
...
function dfn_ajax_admin_get_slots(): void {
    ...
    if ( $event_id <= 0 || empty( $date ) ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Parametri mancanti.', 'dfn-theme' ) ) );
    }
    ...
    foreach ( $slots as $slot ) {
        ...
        foreach ( $bookings as $b ) {
            ...
            if ( $b->order_id ) {
                ...
                if ( $order ) {
                    $fai_cards = $order->get_meta( '_dfn_fai_cards' ) ?: array();
                }
            }
            $bookings_list[] = array(
                'id'               => intval( $b->id ),
...
function dfn_ajax_admin_generate_initial_slots(): void {
    ...
    $event_id = isset( $_POST['event_id'] ) ? intval( $_POST['event_id'] ) : 0;
    if ( $event_id <= 0 ) {
        wp_send_json_error( array( 'message' => esc_html__( 'ID evento non valido.', 'dfn-theme' ) ) );
    }
    ...
    if ( $count > 0 ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Questo evento ha già degli slot generati.', 'dfn-theme' ) ) );
    }
    ...
    if ( $generated > 0 ) {
        ...
        wp_send_json_error( array( 'message' => esc_html__( 'Impossibile generare gli slot. Verifica le date e le fasce orarie impostate nell\'evento.', 'dfn-theme' ) ) );
    }
}
...
function dfn_ajax_admin_add_slot(): void {
    ...
    if ( $event_id <= 0 || empty( $date ) || empty( $time_start ) || empty( $time_end ) || $capacity < 0 || $bonus < 0 ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Dati non validi o mancanti.', 'dfn-theme' ) ) );
    }
    ...
    if ( $exists ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Esiste già uno slot per questo orario di inizio in questa data.', 'dfn-theme' ) ) );
    }
    ...
    if ( $inserted ) {
        wp_send_json_success( array( 'message' => esc_html__( 'Slot orario aggiunto con successo.', 'dfn-theme' ) ) );
    } else {
        wp_send_json_error( array( 'message' => esc_html__( 'Errore nel salvataggio dello slot.', 'dfn-theme' ) ) );
    }
}
...
function dfn_ajax_admin_update_slot(): void {
    ...
    if ( $slot_id <= 0 || $capacity < 0 || $bonus < 0 ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Dati non validi.', 'dfn-theme' ) ) );
    }
    ...
    if ( $updated !== false ) {
        wp_send_json_success( array( 'message' => esc_html__( 'Slot aggiornato con successo.', 'dfn-theme' ) ) );
    } else {
        wp_send_json_error( array( 'message' => esc_html__( 'Impossibile aggiornare lo slot.', 'dfn-theme' ) ) );
    }
}
...
function dfn_ajax_admin_lock_slot(): void {
    ...
    if ( $slot_id <= 0 ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Slot non valido.', 'dfn-theme' ) ) );
    }
    ...
    if ( $updated !== false ) {
        ...
        wp_send_json_error( array( 'message' => esc_html__( 'Errore durante la modifica dello stato di blocco.', 'dfn-theme' ) ) );
    }
}
...
function dfn_ajax_admin_delete_slot(): void {
    ...
    if ( ! $slot ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Slot non trovato.', 'dfn-theme' ) ) );
    }

    if ( intval( $slot->booked_count ) > 0 ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Non puoi eliminare uno slot che contiene delle prenotazioni attive. Sposta prima i visitatori in altri slot.', 'dfn-theme' ) ) );
    }

    $deleted = $wpdb->delete( $table_slots, array( 'id' => $slot_id ), array( '%d' ) );

    if ( $deleted ) {
        wp_send_json_success( array( 'message' => esc_html__( 'Slot eliminato con successo.', 'dfn-theme' ) ) );
    } else {
        wp_send_json_error( array( 'message' => esc_html__( 'Errore nell\'eliminazione dello slot.', 'dfn-theme' ) ) );
    }
}
...
function dfn_ajax_admin_add_booking(): void {
    ...
    if ( $event_id <= 0 || $slot_id <= 0 || empty( $date ) || empty( $first_name ) || empty( $last_name ) ) {
        wp_send_json_error( array( 'message' => esc_html__( 'I campi Nome e Cognome dell\'acquirente sono obbligatori.', 'dfn-theme' ) ) );
    }

    $total_qty = $qty_standard + $qty_fai;
    if ( $total_qty <= 0 ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Specifica almeno un biglietto.', 'dfn-theme' ) ) );
    }
    ...
    try {
        if ( ! function_exists( 'wc_create_order' ) ) {
            wp_send_json_error( array( 'message' => esc_html__( 'WooCommerce non è attivo.', 'dfn-theme' ) ) );
        }
        ...
        if ( $total_discount > 0.00 ) {
            ...
            $item_fee->set_name( sprintf( 'Sconto Soci FAI (%d tessere) - Ins. Manuale', $qty_fai ) );
...
function dfn_ajax_admin_move_booking(): void {
    ...
    if ( $booking_id <= 0 || $from_slot_id <= 0 || $to_slot_id <= 0 ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Parametri non validi.', 'dfn-theme' ) ) );
    }

    if ( $from_slot_id === $to_slot_id ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Lo slot di destinazione è identico a quello di origine.', 'dfn-theme' ) ) );
    }
    ...
    if ( ! $assoc ) {
        $wpdb->query( 'ROLLBACK' );
        wp_send_json_error( array( 'message' => esc_html__( 'Associazione prenotazione-slot non trovata.', 'dfn-theme' ) ) );
    }
    ...
    if ( ! $to_slot ) {
        $wpdb->query( 'ROLLBACK' );
        wp_send_json_error( array( 'message' => esc_html__( 'Slot di destinazione non trovato.', 'dfn-theme' ) ) );
    }

    if ( intval( $to_slot->is_locked ) === 1 ) {
        $wpdb->query( 'ROLLBACK' );
        wp_send_json_error( array( 'message' => esc_html__( 'Lo slot di destinazione è bloccato.', 'dfn-theme' ) ) );
    }
    ...
    if ( $persons > $available_space ) {
        $wpdb->query( 'ROLLBACK' );
        wp_send_json_error( array( 'message' => sprintf( esc_html__( 'Spazio insufficiente nello slot di destinazione. Posti necessari: %d, posti disponibili: %d.', 'dfn-theme' ), $persons, $available_space ) ) );
    }
    ...
    $wpdb->update(
        $table_booking_slots,
        array( 'slot_id' => $to_slot_id ),
        array( 'booking_id' => $booking_id, 'slot_id' => $from_slot_id ),
        array( '%d' ),
        array( '%d', '%d' )
    );
    ...
    wp_send_json_success( array( 'message' => esc_html__( 'Prenotazione spostata con successo.', 'dfn-theme' ) ) );
}
...
function dfn_ajax_admin_delete_booking(): void {
    ...
    if ( $booking_id <= 0 ) {
        wp_send_json_error( array( 'message' => esc_html__( 'Prenotazione non valida.', 'dfn-theme' ) ) );
    }
    ...
    wp_send_json_success( array( 'message' => esc_html__( 'Prenotazione cancellata con successo.', 'dfn-theme' ) ) );
}
