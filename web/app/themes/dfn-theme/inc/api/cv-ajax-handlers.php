...
function cv_search_customers_ajax()
{
    ...
    $results = [];
    $emails_found = [];
    $users = new WP_User_Query([ 'search' => '*' . esc_attr($term) . '*', 'search_columns' => [ 'user_login', 'user_email', 'user_nicename', 'display_name' ], 'number' => 10 ]);
...
function cv_get_customer_data_ajax()
{
    ...
    if (is_numeric($customer_id_raw) && $customer_id_raw > 0) {
        ...
        try {
            ...
            wp_send_json_error([ 'message' => 'Cliente non trovato.' ]);
        }
        ...
        if (empty($phone) || empty($first_name)) {
            $last_orders = wc_get_orders([ 'customer_id' => intval($customer_id_raw), 'limit' => 1, 'orderby' => 'date', 'order' => 'DESC' ]);
...
function cv_fetch_report_data_ajax()
{
        ...
        for ($i = 1; $i <= $qty_prodotto; $i++) {
            if ($order->get_meta('_cv_ticket_validato_' . $i) === 'yes') {
                ...
                if ($op_id) {
                    ...
                    if (!isset($global_validators[$nome_op])) {
                        $global_validators[$nome_op] = 0;
                    }
    ...
    wp_send_json_success([ 'tot_venduti' => $tot_venduti, 'tot_checkin' => $tot_checkin, 'tot_residui' => $tot_residui, 'tot_rimanenti' => $tot_rimanenti, 'html_leaderboard' => $html_leaderboard, 'html_table_rows' => $table_rows, 'html_pagination' => $html_pagination ]);
}
...
function cv_track_ticket_action_ajax()
{
    ...
    $history[] = [ 'time' => current_time('mysql'), 'action' => $action ];
    if (count($history) > 50) {
...
function cv_process_live_scan_ajax()
{
    ...
    if (! $order->has_status([ 'processing', 'completed' ])) {
        wp_send_json_error([ 'message' => "<span style='font-size:30px;'>🚫 ORDINE NON VALIDO</span><br><br>I biglietti di questo ordine sono stati disattivati poiché l'ordine risulta annullato, rimborsato o non pagato." ]);
    }
