...
function dfn_ajax_get_event_dates()
{
    ...
    if ($product_id <= 0) {
        wp_send_json_error([ 'message' => __('ID prodotto non valido.', 'dfn-theme') ]);
    }

    $event = dfn_db_get_event_by_product($product_id);
    if (! $event) {
        wp_send_json_error([ 'message' => __('Evento non associato a questo prodotto.', 'dfn-theme') ]);
    }
...
function dfn_ajax_get_available_slots()
{
    ...
    if ($product_id <= 0 || empty($date)) {
        wp_send_json_error([ 'message' => __('Parametri incompleti.', 'dfn-theme') ]);
    }
