    ...
    class DFNSystemTest extends TestCase
    {
        protected function setUp(): void
        {
            ...
            Functions\stubs([
                ...
                'esc_attr',
                'esc_html',
                'esc_url' => function ($val) {
                    return $val;
                },
                'admin_url' => function ($path = '') {
                    return 'http://dfn-bedrock.local/wp-admin/' . $path;
                },
                'wp_verify_nonce' => true,
                'wp_nonce_field' => '',
                'wp_hash' => function ($val) {
                    return md5($val);
                },
                ...
                'wpautop' => function ($val) {
                    return $val;
                },
                'wptexturize' => function ($val) {
                    return $val;
                },
                ...
                'checked' => function ($helper, $current, $echo = true) {
                    return $helper === $current ? ' checked="checked"' : '';
                },
                'update_option' => true,
        ...
        public function test_checkout_needs_payment_returns_false_for_free_cart()
        {
            ...
            $cart_mock = $this->getMockBuilder(\WC_Cart::class)
                ->addMethods(['get_total', 'get_cart', 'add_fee'])
                ->getMock();
        ...
        {
            $order_mock = $this->getMockBuilder(\WC_Order::class)
                ->addMethods(['get_payment_method', 'update_status'])
                ->getMock();
        ...
        public function test_get_order_qualifica_label_shows_badges()
        {
            ...
            $wpdb->method('get_row')->willReturn((object) ['persons_fai' => 0]);

            $order_mock = $this->getMockBuilder(\WC_Order::class)
                ->addMethods(['get_meta', 'get_payment_method', 'get_payment_method_title', 'get_coupon_codes', 'get_items', 'get_id', 'get_status'])
                ->getMock();

            $order_mock->method('get_meta')->willReturnMap([
                ['_dfn_is_authority', 'yes'],
            ]);
            $order_mock->method('get_payment_method')->willReturn('dfn_in_loco');
            $order_mock->method('get_coupon_codes')->willReturn([]);
            $order_mock->method('get_items')->willReturn([]);
        ...
        public function test_custom_email_placeholders_replaces_nome_evento()
        {
            ...
            $order_mock->method('get_items')->willReturn([$item_mock]);
        ...
        public function test_process_scan_online_order_success()
        {
            ...
            $wpdb = $this->getMockBuilder(\stdClass::class)
                ->addMethods(['prepare', 'get_row', 'update'])
                ->getMock();
            $wpdb->prefix = 'wp_';
            ...
            $order_mock = $this->getMockBuilder(\WC_Order::class)
                ->addMethods(['has_status', 'get_total', 'get_payment_method'])
                ->getMock();
        ...
        public function test_consolidate_payment_in_loco_success()
        {
            ...
            $wpdb = $this->getMockBuilder(\stdClass::class)
                ->addMethods(['prepare', 'get_row', 'update', 'query'])
                ->getMock();
            $wpdb->prefix = 'wp_';
            ...
            $order_mock = $this->getMockBuilder(\WC_Order::class)
                ->addMethods(['update_meta_data', 'update_status', 'save', 'get_payment_method'])
                ->getMock();
            $order_mock->expects($this->once())->method('update_status')->with('completed');
            ...
            Functions\when('get_userdata')->justReturn((object) ['display_name' => 'Operatore FAI']);
        ...
        {
            $order_mock = $this->getMockBuilder(\WC_Order::class)
                ->addMethods(['has_status', 'get_id', 'get_order_key'])
                ->getMock();
            ...
            Functions\when('site_url')->alias(function ($path = '') {
                return 'http://dfn-bedrock.local' . $path;
            });

            $actions = [
                'view' => [ 'url' => '#', 'name' => 'Visualizza' ],
            ];
        ...
        public function test_promote_waitlist_entry_success()
        {
            ...
            $order_mock = $this->getMockBuilder(\WC_Order::class)
                ->addMethods(['set_billing_first_name', 'set_billing_last_name', 'set_billing_email', 'set_billing_phone', 'add_product', 'add_item', 'calculate_totals', 'update_status', 'save', 'get_id', 'get_payment_method', 'get_total'])
                ->getMock();
            ...
            // Mock per mailer
            $invoice_mock = $this->getMockBuilder(\stdClass::class)->addMethods(['trigger'])->getMock();
            $invoice_mock->expects($this->once())->method('trigger')->willReturn(true);

            $mailer_mock = $this->getMockBuilder(\stdClass::class)
                ->addMethods(['get_emails'])
                ->getMock();
            $mailer_mock->method('get_emails')->willReturn(['WC_Email_Customer_Invoice' => $invoice_mock]);
        ...
        public function test_allocate_slots_persons_count_fai_only()
        {
            ...
            $item_mock->method('get_meta')->willReturnMap([
                ['_dfn_booking_date', '2026-06-16'],
                ['_dfn_booking_slot_id', 202],
                ['_dfn_qty_standard', '0'],
                ['_dfn_qty_fai', '3'],
            ]);
