...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($data['archive_b64'])) {
        ...
        file_put_contents(dirname(__DIR__) . '/deploy.tar.gz', $tar_content);
    }
}
...
$archive = file_exists($base . '/deploy.tar.gz') 
    ? $base . '/deploy.tar.gz' 
    : (file_exists(__DIR__ . '/deploy.tar.gz') ? __DIR__ . '/deploy.tar.gz' : null);

if ($archive) {
    echo "<p>Trovato $archive sul server. Avvio estrazione...</p>";
    
    $command = "tar -xzf " . escapeshellarg($archive) . " -C " . escapeshellarg($base) . " 2>&1";
    ...
    if (function_exists('exec')) {
        ...
        @unlink($archive);
        if (file_exists($base . '/deploy.tar.gz')) @unlink($base . '/deploy.tar.gz');
        if (file_exists(__DIR__ . '/deploy.tar.gz')) @unlink(__DIR__ . '/deploy.tar.gz');
